...
use App\Services\SentryDataScrubber;
use Sentry\Event;
...
return [
    ...
    'release' => env('SENTRY_RELEASE', trim(exec('git --git-dir '.base_path('.git').' log --pretty="%h" -n1 HEAD'))) ?: null,
    ...
    // Kept false: Invoicius is a billing system and this flag would additionally send the
    // user's IP address and the currently authenticated user's identity with every event.
    'send_default_pii' => env('SENTRY_SEND_DEFAULT_PII', false),

    // Scrubs billing PII (IBAN, DIC/ICO, client emails, invoice amounts, ...) from request
    // bodies and breadcrumbs before the event leaves the server. See App\Services\SentryDataScrubber.
    'before_send' => static fn (Event $event): ?Event => (new SentryDataScrubber)($event),
