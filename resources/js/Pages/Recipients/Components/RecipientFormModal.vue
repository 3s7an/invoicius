    ...
    <Modal :show="show" max-width="xl" @close="emit('close')">
        ...
        <div class="flex shrink-0 items-start gap-3 border-b border-gray-200 px-6 py-4">
            <div class="flex-1">
                ...
                <p class="mt-0.5 text-[13px] text-gray-500">Zadajte fakturačné a kontaktné údaje.</p>
            </div>
            <button
                type="button"
        ...
        <form class="overflow-y-auto px-6 pb-5 pt-1" @submit.prevent="submitModal">
            ...
            <div class="flex items-center gap-2.5 mb-3.5 mt-[22px]">
                <span class="text-[11px] font-semibold uppercase tracking-[0.08em] text-gray-400 whitespace-nowrap">Firemné údaje</span>
                <span class="flex-1 h-px bg-gray-200" />
            </div>

            <div class="flex flex-col gap-3.5">
                <div>
                    <InputLabel for="modal-name" value="Názov firmy" />
                ...
                <div class="grid grid-cols-2 gap-3.5">
                    <div>
                        ...
                        <TextInput id="modal-ico" v-model="form.ico" type="text" class="mt-1 block w-full" placeholder="12 345 678" />
                        <InputError class="mt-1" :message="form.errors.ico" />
                    </div>
                    <div>
                        <InputLabel for="modal-dic" value="DIČ" />
                        <TextInput id="modal-dic" v-model="form.dic" type="text" class="mt-1 block w-full" placeholder="2012345678" />
                        <InputError class="mt-1" :message="form.errors.dic" />
                    </div>
                </div>

                <div>
                    <InputLabel for="modal-ic_dph" value="IČ DPH" />
                    <TextInput id="modal-ic_dph" v-model="form.ic_dph" type="text" class="mt-1 block w-full" placeholder="SK2012345678" />
                    <p class="mt-1 text-xs text-gray-500">Vyplňte len ak je firma platiteľom DPH.</p>
                    <InputError class="mt-1" :message="form.errors.ic_dph" />
                </div>

                <div>
                    <InputLabel for="modal-iban" value="IBAN" />
            ...
            <!-- ── Adresa ── -->
            <div class="flex items-center gap-2.5 mb-3.5 mt-[22px]">
                <span class="text-[11px] font-semibold uppercase tracking-[0.08em] text-gray-400 whitespace-nowrap">Adresa</span>
                <span class="flex-1 h-px bg-gray-200" />
            </div>
            ...
            <div class="flex flex-col gap-3.5">
                <div class="grid grid-cols-[2fr_1fr] gap-3.5">
                    <div>
                        <InputLabel for="modal-street" value="Ulica" />
                        <div class="relative mt-1">
                    ...
                    <div>
                        <InputLabel for="modal-street_num" value="Číslo" />
                        <TextInput id="modal-street_num" v-model="form.street_num" type="text" class="mt-1 block w-full" placeholder="1" />
                ...
                <div class="grid grid-cols-[2fr_1fr] gap-3.5">
                    <div>
                        <InputLabel for="modal-city" value="Mesto" />
                        <TextInput id="modal-city" v-model="form.city" type="text" class="mt-1 block w-full" placeholder="Bratislava" />
                        <InputError class="mt-1" :message="form.errors.city" />
                    </div>
                    <div>
                        <InputLabel for="modal-zip" value="PSČ" />
                        <TextInput id="modal-zip" v-model="form.zip" type="text" class="mt-1 block w-full" placeholder="811 01" />
                        <InputError class="mt-1" :message="form.errors.zip" />
                    </div>
                </div>

                <div>
                    <InputLabel for="modal-state" value="Krajina" />
                    <AppSelect
                        inputId="modal-state"
                        v-model="form.state"
                        :options="countries"
